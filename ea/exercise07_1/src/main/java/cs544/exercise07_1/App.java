...
public class App {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            List<Flight> flights = session.createQuery("Select f from Flight f join f.airplane a " +
                    "where a.capacity > 500 and f.origin.country = :origin")
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Airline> airlines = session.createQuery("Select a from Airline a join a.flights f where " +
                    "f.airplane.model = :model")
        ...
        try {
            ...
            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("from Flight f where f.airline.name != :name" +
                    " and f.airplane.model = :flightnr")
                    .setParameter("name","Star Alliance")
                    .setParameter("flightnr","747")
                    .list();
        ...
        try {
            ...
            // Date
            Date date = new Date(2009,07,8,12,00);




            @SuppressWarnings("unchecked")
            List<Flight> flights = session.createQuery("from Flight f where" +
                    " f.departureDate < :date " +
                    "and hour(departureTime) < hour(:time)")
            .setParameter("date",date)
            .setParameter("time","12:00:00")
            .list();
